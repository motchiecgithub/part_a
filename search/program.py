...
def search(input: dict[tuple, tuple]) -> list[tuple]:
    ...
    while blues:
        curr = open_lst.get()
        test_result.append(curr[1])
        # generate moves and choose optimal moves
        moves = expand(curr[1], reds[curr[1]][1])
        reds.pop(curr[1])
        min_h = MAX_HEURISTIC
        next_move = []
        power = 0
        ...
        direction.append(HEX_NEIGHBORS[moves.index(next_move)])
    result = []
    for i in range(0, len(test_result)):
        result.append((test_result[i][0], test_result[i][1], direction[i][0], direction[i][1]))
